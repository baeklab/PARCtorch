...
class UNetDownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding_mode='zeros'):
        ...
        self.doubleConv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size // 2, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv2d(out_channels, out_channels, kernel_size=1, padding=0, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2)
        )
...
class UNetUpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, skip_channels=0, kernel_size=3, padding_mode='zeros', use_concat=True):
        ...
        self.doubleConv = nn.Sequential(
            nn.Conv2d(conv_in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size // 2, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv2d(out_channels, out_channels, kernel_size=1, padding=0, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2)
        )
...
class UNet(nn.Module):
    ...
    """
    def __init__(self, block_dimensions, input_channels, output_channels, kernel_size=3, padding_mode='zeros',
        ...
        self.doubleConv = nn.Sequential(
            nn.Conv2d(input_channels, block_dimensions[0], kernel_size=kernel_size, padding=kernel_size // 2, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv2d(block_dimensions[0], block_dimensions[0], kernel_size=1, padding=0, padding_mode=padding_mode),
            nn.LeakyReLU(negative_slope=0.2)
        )
        ...
        skip_connection_channels = [in_channels]      # List to store channels for skip connections
