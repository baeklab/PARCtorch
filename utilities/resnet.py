...
class ResNetBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, padding_mode: str):
...
class ResNet(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, block_dimensions: List[int], 
                 kernel_size: int = 3, pooling: bool = False, padding_mode: str = 'constant'):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, block_dimensions[0], kernel_size=kernel_size, 
                      padding=0, padding_mode="zeros"),
            nn.ReLU()
        )
        self.conv2 = nn.Sequential(
        ...
        for i in range(len(block_dimensions)):
            ...
            res_block = ResNetBlock(in_channels=in_channels, out_channels=block_dimensions[i],
