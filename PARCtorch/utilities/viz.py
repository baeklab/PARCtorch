...
def save_gifs_for_channels(predictions, channels, cmaps, filename_prefix="predictions", interval=0.1, batch_idx=0):
    ...
    for i, channel_name in enumerate(channels):
        ...
        for t in range(prediction_sequence.shape[0]):
            ...
            frame = prediction_sequence[t, i, :, :].cpu().numpy()  # Get the i-th channel
...
def save_gifs_with_ground_truth(predictions, ground_truth, channels, cmaps, filename_prefix="comparison", interval=0.1, batch_idx=0):
    """
    ...
    prediction_sequence = predictions[:, batch_idx].cpu()  # Shape: (timesteps, channels, height, width)
    ground_truth_sequence = ground_truth[:, batch_idx].cpu()  # Shape: (timesteps, channels, height, width)
