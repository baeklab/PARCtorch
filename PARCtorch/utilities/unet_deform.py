...
class DoubleConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, padding_mode="zeros", use_deform=False, deform_groups=1):
        ...
        if use_deform:
            ...
            self.conv1 = DeformConv2d(
                ...
                bias=False  # DeformConv2d does not support bias
            )
        else:
    ...
    def forward(self, x):
        if self.use_deform:
            # Generate offsets using the offset convolution layer
...
class UNetDownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, padding_mode="zeros", use_deform=False, deform_groups=1):
        ...
        # Double convolution with optional deformable convolution
        self.doubleConv = DoubleConv(in_channels, out_channels, kernel_size, padding_mode, use_deform, deform_groups)
...
class UNetUpBlock(nn.Module):
    ...
    def __init__(
        ...
        # Determine the number of input channels for doubleConv
        conv_in_channels = in_channels + skip_channels if use_concat else in_channels

        # Double convolution with optional deformable convolution
        self.doubleConv = DoubleConv(conv_in_channels, out_channels, kernel_size, padding_mode, use_deform, deform_groups)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        # Default to using concatenation if not specified
        if self.up_block_use_concat is None:
            self.up_block_use_concat = [True] * num_up_blocks
        else:
            assert len(self.up_block_use_concat) == num_up_blocks, "Length of up_block_use_concat must match the number of up blocks."

        in_channels = block_dimensions[-1]
        skip_idx_counter = 0

        for idx in range(num_up_blocks):
            out_channels = block_dimensions[-(idx + 2)] if idx != num_up_blocks - 1 else output_channels
            use_concat = self.up_block_use_concat[idx]
            skip_channels = skip_connection_channels[self.skip_connection_indices[skip_idx_counter]] if use_concat else 0
        ...
        self.finalConv = nn.Sequential(
            nn.Conv2d(output_channels, output_channels, kernel_size=1, padding=0, padding_mode=padding_mode),
