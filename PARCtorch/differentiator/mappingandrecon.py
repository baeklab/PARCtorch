...
class MappingAndRecon(nn.Module):
    def __init__(self, n_base_features=128, n_mask_channel=1, 
                 output_channel=1, padding_mode="zeros",
                 add_noise=True):
        super(MappingAndRecon, self).__init__()
        self.add_noise = add_noise
        
        # Initialize SPADE generator unit
        self.spade = SPADEGeneratorUnit(in_channels=n_base_features, mask_channels=n_mask_channel,
                                        out_channels=n_base_features, padding_mode=padding_mode,
                                        kernel_size=1)
        
        # Initialize ResNet block
        self.resnet = ResNet(n_base_features, [n_base_features, n_base_features], kernel_size=1, 
                             pooling=False, padding_mode=padding_mode)
        # Final convolution layer
        self.conv_out = nn.Conv2d(in_channels=n_base_features, out_channels=output_channel,
