...
class FiniteDifference(nn.Module):
    ...
    """
    def __init__(self, channel_size=1, filter_1d=np.array([-1.0, 1.0]), padding_mode="replicate", device="cuda"):
        ...
        self.register_buffer("dy_filter", dy_filter.repeat(channel_size, 1, 1, 1).to(device))  # [C,1,filter_size,1]
         
        # Initialize dx_conv weights
        dx_filter = torch.tensor(self.filter_1d, dtype=torch.float32).view(1, 1, 1, self.filter_size)
        self.register_buffer("dx_filter", dx_filter.repeat(channel_size, 1, 1, 1).to(device))  # [C,1,1,filter_size]
