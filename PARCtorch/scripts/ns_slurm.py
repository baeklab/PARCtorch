...
def parse_args():
    ...
    parser.add_argument('--min_max_output', type=str, default='../data/ns_min_max.json',
    ...
    parser.add_argument('--loss_dir', type=str, default='../loss', help='Directory to save loss history')
    parser.add_argument('--log_file', type=str, default='training.log', help='Log file name')
    parser.add_argument('--visualize', action='store_true', help='Visualize a batch before training')
...
def visualize_data(train_loader, device):
    ...
    for batch in train_loader:
        ...
        channel_names = ['Pressure (P)', 'Reynolds (R)', 'Velocity U', 'Velocity V']
...
# Training Loop
# ---------------------------
def train_model(model, train_loader, criterion, optimizer, num_epochs, save_dir, loss_dir, device):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        logging.info(f"Epoch [{epoch}/{num_epochs}], Average Loss: {epoch_loss:.7f}")
        
        # Save model checkpoint every 10 epochs
        if epoch % 10 == 0 or epoch == num_epochs:
            model_save_path = os.path.join(save_dir, f"ns_model_epoch_{epoch}.pth")
...
def main():
    ...
    # Create DataLoader
    train_loader = create_dataloader(args.train_dirs[0], args.min_max_output, args.batch_size, device)
    ...
    # Start Training
    train_model(model, train_loader, criterion, optimizer, args.num_epochs, args.save_dir, args.loss_dir, device)
