...
class DataDrivenIntegrator(nn.Module):
    def __init__(self, n_io: int, n_base_features: int, kernel_size: int=1, padding_mode: str="constant", spade_random_noise: bool=True, **kwarg):
        '''
        Args
        ...
        self.spade = SPADEGeneratorUnit(n_base_features, n_base_features, n_io, kernel_size, 3, padding_mode)
        self.resnet = ResNet(n_base_features, [n_base_features, n_base_features], kernel_size, False, padding_mode)
    ...
    def forward(self, update, current):
        ...
        '''
        out = F.pad(update, (self.pad_size, self.pad_size, self.pad_size, self.pad_size), self.padding_mode)
        out = self.conv1(out)
        out = F.relu(out)
        out = F.pad(out, (self.pad_size, self.pad_size, self.pad_size, self.pad_size), self.padding_mode)
        out = self.conv2(out)
        out = F.relu(out)
